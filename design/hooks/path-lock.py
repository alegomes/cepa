#!/usr/bin/env python3
"""PreToolUse hook: enforce per-agent path-write locks (design topology).
...
The design topology produces ARTIFACTS, never code. Every worker is locked to
docs/design/** (the design-critic to docs/design/reviews/**). The lead writes
nothing but its own expertise file — it routes and synthesizes.
...
ALLOWED_WRITES = {
    # lead — routes, never produces
    "orchestrator":         [],
    "design-lead":          [],

    # workers — locked to the design-artifact tree
    "ux-architect":         ["docs/design/**"],
    "visual-designer":      ["docs/design/**"],
    "design-system-keeper": ["docs/design/**"],
    "prototyper":           ["docs/design/**"],
    "design-critic":        ["docs/design/reviews/**"],
}
...
def detect_agent(payload: dict) -> str:
    ...
    caller. The value is plugin-namespaced (e.g. "design:visual-designer"); we
