...
1. **Locate.** Run `python3 "${CLAUDE_PLUGIN_ROOT}/hooks/session-worktrees.py"`
   ...
   branch — if it says "will CONFLICT," warn the user up front so step 4 is no
   surprise.
...
3. **Confirm both sides are committed.** The session worktree's work must be
...
4. **Green-gate the source.** Read `<worktree-path>/.claude/last-build.json`.
...
5. **Merge.** `git merge --no-ff session/<slice>` into the current branch.
   - **Clean merge** → optionally offer to name it: this is the honest moment to
     ...
     subject (amend the merge commit message). Then proceed to step 6.
   - **Conflict** → STOP. Do not auto-resolve. Report the conflicted files
     ...
     `/common:worktree-merge <slice>` to do the cleanup (step 6). This is the
     expected, healthy outcome when two slices touched the same code.

6. **Prune.** Once the merge is committed:
