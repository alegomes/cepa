#!/usr/bin/env python3
"""PreToolUse hook for the book topology — the BASH half of the path-lock.
...
/tmp/book-bash-pathlock-uncovered.log) so the gap is visible, never
...
def _load_pathlock_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "book_path_lock", str(here / "path-lock.py")
    )
...
def log_uncovered(command: str, agent: str) -> None:
    log_path = os.environ.get(
        "BASH_PATHLOCK_COVERAGE_LOG", "/tmp/book-bash-pathlock-uncovered.log"
    )
    try:
...
def main():
    ...
    try:
        ...
        print("[book bash-path-lock] could not parse hook payload; allowing",
              file=sys.stderr)
        sys.exit(0)
    ...
    print(
        f"[book bash-path-lock] BLOCKED: agent {agent!r} attempted a "
