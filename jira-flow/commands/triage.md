...
1. **ALREADY-IMPLEMENTED** — the behavior already exists in the codebase → propose moving to **In Review** (`defaults.status_map.in_review`). The card carries an Implementation Summary built from the code/git evidence. **Triage proves nothing** — it routes by evidence so the real proof gate (`/jira-flow:prove`) has a queue to work on.
2. **READY** — refined enough to start (clear outcome + acceptance criteria + bounded scope) → propose moving to **To Do** (`defaults.status_map.to_do`), where `/jira-flow:drain` can pick it up.
...
This is the grooming counterpart to the execution sweeps. `/jira-flow:drain` *builds* cards from `to_do`; `/jira-flow:prove-drain` *proves* cards in `in_review`; `/jira-flow:triage` *sorts the backlog into those lanes*. It is read-heavy and writes nothing until you confirm.
...
- **Triage ≠ proof.** Moving a card to In Review here means "looks done, queue it for proof," never "proven." Always end by pointing the freshly-moved cards at `/jira-flow:prove-drain`. Don't let triage's confidence leak into a done claim.
...
2. **ALREADY-IMPLEMENTED** — every *verbatim* acceptance criterion came back IMPLEMENTED with `file:line` + commit, AND the cited code targets the AC at the surface it's written at. (Mixed/PARTIAL does **not** qualify — name the gap and fall through; a partially-done card is usually READY or NEEDS-DECISION, not done.) For each Bucket-1 card also record two attributes, carried into steps 5–8 so the proof gate isn't blindsided:
   ...
   - **not-perturbable flag** — set it for cards whose "done" can't be shown by breaking prod code and re-running a test (seed / reference data, pure configuration). Their confidence comes from inspection/query, not proof — flag them so `/jira-flow:prove-drain` treats them as inspect-not-prove instead of bouncing them UNPROVEN.
...
For IMPLEMENTED rows, show the specific `file:line` + commit in EVIDENCE, and the per-card **proof-strategy hint** (and the `not-perturbable` marker, if set) in the PROOF HINT column — that's what `/jira-flow:prove-drain` reads to attack the card correctly. For OBSOLETE/dedup rows, name the canonical key and its real state. Honesty over tidiness: if a card is PARTIAL, say so in the reason rather than rounding it up to IMPLEMENTED.
...
- **→ In Review.** Moving into a review status requires an Implementation Summary (see `atlassian-expert`'s "Transition to Review with Implementation Summary"). Assemble it from the `Explore` evidence — the `file:line`s satisfying each criterion and the commit SHAs — and **mark it triage-sourced**, e.g. a header line "Routed by /jira-flow:triage from code evidence; not yet proven — pending /jira-flow:prove." Append the card's **proof-strategy hint** and, if set, the **not-perturbable** marker so `/jira-flow:prove-drain` attacks the card correctly instead of bouncing it. Then delegate:
  ...
  >
  > **Proof strategy (for /jira-flow:prove):** <hint, e.g. "bug — regression test must go RED at base_commit" / "altitude — AC is at the REST surface, needs an E2E, expect completion-auditor" / "has contract+E2E — proof should be cheap">
...
- **Next steps:** "Run `/jira-flow:prove-drain` to prove the cards just moved to In Review (triage routed them by evidence, it did not prove them). Run `/jira-flow:drain` to build the To Do cards." If `--dry-run`, note that nothing was written.
...
- **In Review here is a candidacy, not a verdict.** Triage routes by code evidence; `/jira-flow:prove` is what proves load-bearing behavior at the surface. Never report a triaged-to-Review card as "done."
...
- **Every Bucket-1 card carries a proof-strategy hint** (and a `not-perturbable` marker where it applies) into its Implementation Summary, so `/jira-flow:prove-drain` attacks it correctly — bugs at regression-RED-at-base, REST-altitude ACs via E2E, seed/config by inspection.
