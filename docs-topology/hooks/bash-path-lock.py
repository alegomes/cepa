...
re-declare globs here) and applies it to shell-level writes. This matters more
than usual here: structure-surgeon legitimately uses `git mv` in the declutter
phase, so its allowlist (docs/**, archive/**, ...) governs the move TARGETS.
...
/tmp/docs-bash-pathlock-uncovered.log) so the gap is visible, never silent.
