...
def _load_pathlock_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "hex_path_lock", str(here / "path-lock.py")
    )
...
def _mensagem(n, agent, command, allowed):
    ...
    linhas = [
        f"[build-hex bash-path-lock] NEGADO ({_BALDE_NOME.get(n.balde, n.balde)}): "
        f"agente {agent!r}.",
        f"  Motivo: {n.motivo}",
    ]
    if n.alvos:
    ...
    linhas += [
        ...
        "  Escrever código por Bash para contornar o path-lock é bypass de delegação.",
        "",
...
def main():
    ...
    roles, extra = pl.load_config(project_root)
