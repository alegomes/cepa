...
For the opposite direction (code changed, spec needs update), use `/hex-backend:spec-e2e <endpoint>` to re-author the spec with `--preview` to see the diff. For 3-way alignment audit (no propagation), use `/hex-backend:audit-e2e`.
...
- `<METHOD> <path>` → mode = `single`. Same parsing rules as `/hex-backend:spec-e2e`.
...
In `single` mode: find the section matching the requested endpoint. If absent → error: "No spec section for `<METHOD> <path>`. Author one first with `/hex-backend:spec-e2e <METHOD> <path>`."
...
- **Spec is authoritative; don't second-guess it.** If a spec assertion seems wrong, the answer is to update the spec via `/hex-backend:spec-e2e --preview`, not to skip the test or weaken the assertion in code. Re-sync re-aligns tests; it doesn't rewrite the spec.
...
- **Don't change the spec doc.** This is the propagation direction; the spec is the input. Spec edits go through `/hex-backend:spec-e2e`.
