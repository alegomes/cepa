...
def main():
    ...
    try:
        ...
        print("[build-team path-lock] could not parse hook payload; allowing", file=sys.stderr)
        sys.exit(0)
    ...
    if allowed is None:
        print(
            f"[build-team path-lock] BLOCKED: unknown agent {agent!r} attempted "
            f"{tool_name} on {file_path}. Add {agent!r} to "
            f"build-team/hooks/path-lock.py if this is intentional.",
            file=sys.stderr,
        )
        sys.exit(2)
    ...
    print(
        f"[build-team path-lock] BLOCKED: agent {agent!r} cannot {tool_name} {file_path}.\n"
