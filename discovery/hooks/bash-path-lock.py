#!/usr/bin/env python3
"""PreToolUse hook for the discovery topology — the BASH half of the path-lock.
...
coverage log (BASH_PATHLOCK_COVERAGE_LOG, default /tmp/discovery-bash-pathlock-
...
def _load_pathlock_module():
    ...
    spec = importlib.util.spec_from_file_location(
        "discovery_path_lock", str(here / "path-lock.py")
    )
...
def log_uncovered(command: str, agent: str) -> None:
    log_path = os.environ.get(
        "BASH_PATHLOCK_COVERAGE_LOG", "/tmp/discovery-bash-pathlock-uncovered.log"
    )
    try:
...
def main():
    ...
    try:
        ...
        print("[discovery bash-path-lock] could not parse hook payload; allowing",
              file=sys.stderr)
        sys.exit(0)
    ...
    print(
        f"[discovery bash-path-lock] BLOCKED: agent {agent!r} attempted a "
