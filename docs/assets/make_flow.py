...
Two cuts, both stated in the README caption:
  - the axis starts on START, when work on the project actually began (the
    cards created before that were a parked backlog);
  - the days in AWAY (engineer travelling, nothing ran) are removed from the
    axis and marked with a visible break.
"""
import csv
...
AWAY = {date(2026, 9, n) for n in (5, 6, 7, 8)}
start = START
end = max([d(r["created"]) for r in rows] + [d(r["resolved"]) for r in rows if r["resolved"]])
days = [t for t in (start + timedelta(n) for n in range((end - start).days + 1)) if t not in AWAY]
...
brk = days.index(max(AWAY) + timedelta(1))
xb = (x(brk - 1) + x(brk)) / 2
o.append(f'<line x1="{xb:.1f}" x2="{xb:.1f}" y1="{T}" y2="{H-B}" stroke="#ffffff" stroke-width="5"/>')
o.append(f'<line x1="{xb:.1f}" x2="{xb:.1f}" y1="{T}" y2="{H-B}" stroke="#111827" stroke-dasharray="4 3"/>')
o.append(f'<text x="{xb-6:.1f}" y="{T+12}" text-anchor="end" fill="#111827">{len(AWAY)} days away, removed</text>')
...
before = sum(1 for r in rows if r["outcome"] == "done" and d(r["resolved"]) < START)
print(f"{start}..{end} days_on_axis={len(days)} scope={scope[-1]} done={done[-1]} done_before_start={before}")
