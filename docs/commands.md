...
| `/board-flow:next` | `[--plan NOME] [--offline] [--sync]` | Answers "what do I do next?" at any moment. Reads the single-track execution plan (`.claude/programs/<project_key>/plan.yaml`, schema `common/plan-schema.yaml`), reconciles it against the live board, and names **one** next step with the `why` recorded when it was prioritised. Separates the two kinds of "next" that compete in silence: **pending human action** (the aggregated `Human validation route` — validate by hand, approve a PR, rotate a secret) and **the next card**. Read-only unless `--sync`, which writes the reconciliation back (merging, never overwriting); `--offline` skips the Jira round-trip. Never invents an order: with no plan, or for cards outside it, it says so and points at `/board-flow:triage`. |
