...
| `/common:spec` | `<idea in prose> \| --continuar <slug> \| --fechar <slug> \| --lista` | Interviews you in rounds until a high-level idea becomes a buildable specification, recording each answer in a versioned markdown in the repo. Declares it ready only when every success criterion has an observable surface (http/cli/ui/event/domain/application) and a failing test declared on it. Writes no code, needs no Jira (offers Epic + Stories at the end if board-flow is installed). Resumable. |
